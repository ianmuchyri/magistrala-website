...
For detailed instructions on managing metadata, including working with interactive maps for location and perimeter data, see the [Metadata Management Guide](../metadata).
...
For detailed instructions on each protocol, see the [Messaging Guide](../../dev-guide/messaging).

### Saving Messages

For a message to be saved on the Magistrala database, a **Rule** must be created in Rules Engine in relation to the specific channel and topic. This has been further discussed in the [**Rules Engine** documentation](../rules-engine).
...
To delete a channel, click the **Delete** button in the row actions of the channels table, or from the channel view page.

A confirmation dialog appears asking you to type the channel's name before deletion is finalized. Use the **copy button** to copy the name, paste it into the input field, then click **Delete** to permanently remove the channel.

![Delete Channel](../../img/clients/channel-delete.png)
