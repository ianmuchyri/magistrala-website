...
Here's an example in which we're updating the domain with the ID `16a4ad70-a385-45c5-b50a-6d7e48d768d7` to have the name `domain_name` instead of `mydomain`, the alias `domain_alias` instead of `myalias`, and adding new metadata:
...
To have the metadata recognized and displayed correctly by the Magistrala UI, use the UI-compatible format with type information:
