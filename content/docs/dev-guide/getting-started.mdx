...
### Fetching the Latest SuperMQ Schema

When SuperMQ publishes upstream schema changes, pull them and regenerate the combined schema with:

```bash
make fetch_supermq
```

This runs `scripts/supermq.sh`, which:

1. Clones the SuperMQ repository (sparse checkout, `docker/` folder only).
2. Replaces `docker/supermq-docker/` with the freshly fetched files.
3. Automatically runs `scripts/combine-schema.sh` to regenerate `combined-schema.zed`.

### How the Combine Script Works

`scripts/combine-schema.sh` merges the two schemas by injecting Magistrala's extensions directly into the upstream SuperMQ schema:
...
3. **Inject into the upstream SuperMQ schema** — Magistrala relations and permissions are inserted after the last SuperMQ group relation/permission; `membership_extension` is spliced into `permission membership` before `organization->admin`.
...
If you only changed `override-schema.zed` and don't need a SuperMQ update:
