...
Any data producer or data consumer - which we refer to here collectively as data agent - or an interrelated system of data agents, can be represented by means of possibly multiple [Magistrala clients, channels and subtopics][architecture]. For example, an OPC-UA server can be represented as a Magistrala client and its nodes can be represented as multiple Magistrala channels or multiple subtopics of a single Magistrala channel. What is more, you can invert the representation: you can represent server as a channel and node as clients. Magistrala platform is meant to empower you with the freedom of expression so you can make a digital representation of any data agent according to your needs.
...
[architecture]: /docs/dev-guide/architecture
