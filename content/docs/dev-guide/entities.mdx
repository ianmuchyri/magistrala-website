...
The API endpoint for interacting with users are described in the [Users API](./api#users).
The CLI for interacting with users are described in the [Users CLI](./cli/users-cli).
...
The API endpoint for interacting with groups are described in the [groups API](./api#groups).
The CLI for interacting with groups are described in the [Groups CLI](./cli/groups-cli).
...
Tags are supported on **Users**, **Domains**, **Groups**, **Clients**, and **Channels**. When listing any of these entities, the `tags` query parameter can be used to filter results by one or more tag values.
...
```
GET /domains/{domainID}/channels?tags=temperature,humidity
```

**AND filter** — return entities tagged both `outdoor` and `sensor`:

```
GET /domains/{domainID}/channels?tags=outdoor%2Bsensor
```
...
`Metadata` is a free-form JSON object (`map[string]interface{}` in Go) available on all entity types. It is used to attach arbitrary structured data to an entity.
